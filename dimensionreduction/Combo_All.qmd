...
Estimate and apply corrections for Cluster 5. There are only signicant differences over time in both groups.
...
There are 5 clusters for Group A. There are 2 x clusters containing 2 objects. Because of dimensionality, these have to be selected as outcome subsets. There are 2 clusters for Group B. The smallest cluster has 3 objects. Because of dimensionality, this has to be selected as an outcome subset.
```{r}

hcdata3.2A = dendro_data_k(res3.2.hclustA, 5)

p = plot_ggdendro(hcdata3.2A,
                  scale.color = cols,
                   direction   = "rl",
                   expand.y    = 0.2) 
p + ylab("\n Link Height") + geom_hline(yintercept = 0.25, linetype = "solid", size = 1) + theme_bw() +
  theme(axis.title.x = element_text(size = 12, family = "Arial", face = "bold"),
        axis.title.y = element_blank(),
        axis.text.x = element_text(size = 10, family = "Arial"),
        axis.text.y = element_text(size = 10, family = "Arial"))
...
clusters3.2A = cutree(res3.2.hclustA, k=5)

# Group 1

names5 = names(clusters3.2A[clusters3.2A == 1])
names3 = names(clusters3.2A[clusters3.2A == 2])
names4 = names(clusters3.2A[clusters3.2A == 3])
names2 = names(clusters3.2A[clusters3.2A == 4])
names1 = names(clusters3.2A[clusters3.2A == 5])

clusters3.2B = cutree(res3.2.hclustB, k=2)

names2B = names(clusters3.2B[clusters3.2B == 1])
names1B = names(clusters3.2B[clusters3.2B == 2])
...
fa_pca_set2B = names2B
```

Perform PCA on Cluster 3. PC1 explains sufficient input variance.
...
Cluster 3 describes variance in a number of inputs and cannot be easily characterised.
...
```
There are 4 immune outcomes selected.

```{r}

fviz_contrib(pca3.2Ares, axes = 1, choice = "ind", fill = "darkgrey", color = "black", top = 10) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
  theme(title = element_blank(),
        axis.title.y = element_text(size = 12, face = "bold"),
...
Perform PCA on Cluster 4. PC1 is sufficient to describe variance in input data.

```{r}

mat3.2A = tmelt_3A[, c("Outcome", "Coefficient", "value")]
mat3.2A = mat3.2A %>% filter(Outcome %in% names4)
...
```
Cluster 4 primarily describes variance in FA3 but other coefficients are also included.
...
fviz_contrib(pca3.2Ares, axes = 1, choice = "var", fill = "darkgrey", color = "black", top = 10) +
  ...
  theme(title = element_blank(),
        ...
        axis.text.y = element_text(size = 10))
```
There are 3 immune outcomes selected to the subset.
```{r}
...
fa_pca_set4 = pcnames1

```
Perform PCA on Cluster 5. The first two PCs are necessary to explain the majority of variance in input data. 
...
PC1 primarily describes FA3. PC2 primarily describes FA1.
```{r}
fviz_contrib(pca3.2Ares, axes = 1, choice = "var", fill = "darkgrey", color = "black", top = 10) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
...
Select 3 outcomes for PC1. Select 3 outcome for PC2.
```{r}
...
fviz_contrib(pca3.2Ares, axes = 2, choice = "ind", fill = "darkgrey", color = "black", top = 10) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
  theme(title = element_blank(),
        axis.title.y = element_text(size = 12, face = "bold"),
...
pc1dat = contrib3.2A.1$data[which(contrib3.2A.1$data$contrib >= 10), ]
pc2dat = contrib3.2A.2$data[which(contrib3.2A.2$data$contrib >= 10), ]
...
```{r}
faclusters = c(fa_pca_set1, fa_pca_set2, fa_pca_set3, names4, fa_pca_set5)

```

Perform PCA on Cluster 1 of Group B. PC1 and PC2 explain sufficient input variance.

```{r}

mat3.2B = tmelt_3B[, c("Outcome", "Coefficient", "value")]
mat3.2B = mat3.2B %>% filter(Outcome %in% names1B)
...
Cluster 1 does not have a simple definition.
...
There are 4 immune outcomes making large contributions to PC1. There are 3 contributing to PC2.
...
fa_pca_set1A = pcnames1

```

```{r}
faclustersB = c(fa_pca_set1B, fa_pca_set2B)
...
AM_C1.1 = glmmTMB(101 - O19 ~ FA*Time + Gender*Time + (1| ID), data = groupA, family = Gamma(link = "log"))
AS_C1.1 = summary(AM_C1.1)

AM_C1.2 = glmmTMB(101 - O37 ~ FA*Time + Gender*Time + (1| ID), data = groupA, family = tweedie(link = "log"))
AS_C1.2 = summary(AM_C1.2)

n1 = length(fa_pca_set1)
v1.1 = varA[19]
v1.2 = varA[37]

# Cluster 2 models

AM_C2.1 = glmmTMB(O20 ~ FA*Time + Gender*Time + (1| ID), data = groupA, family = tweedie(link = "log"))
AS_C2.1 = summary(AM_C2.1)

AM_C2.2 = glmmTMB(O27 ~ FA*Time + Gender*Time + (1| ID), data = groupA, family = tweedie(link = "log"))
AS_C2.2 = summary(AM_C2.2)

n2 = length(fa_pca_set2)
...
AM_C3.1 = glmmTMB(O5 ~ FA*Time + Gender*Time + (1| ID), data = groupA, family = tweedie(link = "log"))
AS_C3.1 = summary(AM_C3.1)

AM_C3.2 = glmmTMB(O25 ~ FA*Time + Gender*Time + (1| ID), data = groupA, family = tweedie(link = "log"))
AS_C3.2 = summary(AM_C3.2)

AM_C3.3 = glmmTMB(94-O15 ~ FA*Time + Gender*Time + (1| ID), data = groupA, family = tweedie(link = "log"))
AS_C3.3 = summary(AM_C3.3)

AM_C3.4 = glmmTMB(101 - O36 ~ FA*Time + Gender*Time + (1| ID), data = groupA, family = tweedie(link = "log"))
AS_C3.4 = summary(AM_C3.4)

n3 = length(fa_pca_set3)
...
AM_C4.1 = glmmTMB(O21 ~ FA*Time + Gender*Time + (1| ID), data = groupA, family = Gamma(link = "log"))
AS_C4.1 = summary(AM_C4.1)

AM_C4.2 = glmmTMB(O18 ~ FA*Time + Gender*Time + (1| ID), data = groupA, family = Gamma(link = "log"))
AS_4.2 = summary(AM_C4.2)

AM_C4.3 = glmmTMB(O29 ~ FA*Time + Gender*Time + (1| ID), data = groupA, family = tweedie(link = "log"))
AS_4.3 = summary(AM_C4.3)

n4 = length(fa_pca_set4)
...
AM_C5.1 = glmmTMB(O3 ~ FA*Time + Gender*Time + (1| ID), data = groupA, family = tweedie(link = "log"))
AS_C5.1 = summary(AM_5.1)

AM_C5.2 = glmmTMB(O17 ~ FA*Time + Gender*Time + (1| ID), data = groupA, family = Gamma(link = "log"))
AS_C5.2 = summary(AM_C5.2)

AM_C5.3 = glmmTMB(O8 ~ FA*Time + Gender*Time + (1| ID), data = groupA, family = tweedie(link = "log"))
AS_C5.3 = summary(AM_C5.3)

AM_C5.4 = glmmTMB(O6 ~ FA*Time + Gender*Time + (1| ID), data = groupA, family = Gamma(link = "log"))
AS_C5.4 = summary(AM_C5.4)

AM_C5.5 = glmmTMB(O16 ~ FA*Time + Gender*Time + (1| ID), data = groupA, family = Gamma(link = "log"))
AS_C5.5 = summary(AM_C5.5)

AM_C5.6 = glmmTMB(O23 ~ FA*Time + Gender*Time + (1| ID), data = groupA, family = tweedie(link = "log"))
AS_C5.6 = summary(AM_C5.6)

n5 = length(fa_pca_set5)
...
BM_C1.1 = glmmTMB(O11 ~ FA*Time + Gender*Time + (1| ID), data = groupB2, family = poisson(link = "log"))
BS_C1.1 = summary(BM_C1.1)

BM_C1.2 = glmmTMB(O14 ~ FA*Time + Gender*Time + (Time| ID), data = groupB2, family = t_family(link = "log"))
BS_C1.2 = summary(BM_C1.2)

BM_C1.3 = glmmTMB(O18 ~ FA*Time + Gender*Time + (Time| ID), data = groupB2, family = Gamma(link = "log"))
BS_C1.3 = summary(BM_C1.3)

BM_C1.4 = glmmTMB(O19 ~ FA*Time + Gender*Time + (1| ID), data = groupB2, family = Gamma(link = "log"))
BS_C1.4 = summary(BM_C1.4)

n1B = length(fa_pca_set1B)
...
BM_C2.1 = glmmTMB(O9 ~ FA*Time + Gender*Time + (1| ID), data = groupB2, family = tweedie(link = "log"))
BS_C2.1 = summary(BM_C2.1)

BM_C2.2 = glmmTMB(O8 ~ FA*Time + Gender*Time + (1| ID), data = groupB2, family = nbinom1(link = "log"))
BS_C2.2 = summary(BM_C2.2)

BM_C2.3 = glmmTMB(O15 ~ FA*Time + Gender*Time + (1| ID), data = groupB2, family = poisson(link = "log"))
BS_C2.3 = summary(BM_C2.3)

n1B = length(fa_pca_set2B)
...
combinations = data.frame(emmeans(AM_C1.1, ~ FA * Time))
...
Estimate and apply corrections for Cluster 1. Primarily significant differences within group over time.
```{r}

emmC1.1 = emmeans(AM_C1.1, ~ FA * Time)
emmC1.2 = emmeans(AM_C1.2, ~ FA * Time)
...
clusterC1resB$expestimate = exp(clusterC1resB$estimate)
...
Estimate and apply corrections for Cluster 2. Primarily significant differences within group over time.
```{r}

emmC2.1 = emmeans(AM_C2.1, ~ FA * Time)
emmC2.2 = emmeans(AM_C2.2, ~ FA * Time)
...
Estimate and apply corrections for Cluster 3. Primarily significant differences within group over time.
```{r}

emmC3.1 = emmeans(AM_C3.1, ~ FA * Time)
emmC3.2 = emmeans(AM_C3.2, ~ FA * Time)
emmC3.3 = emmeans(AM_C3.3, ~ FA * Time)
emmC3.4 = emmeans(AM_C3.4, ~ FA * Time)
...
Estimate and apply corrections for Cluster 4. Primarily significant differences within group over time.
```{r}
emmC4.1 = emmeans(AM_C4.1, ~ FA * Time)
emmC4.2 = emmeans(AM_C4.2, ~ FA * Time)
emmC4.3 = emmeans(AM_C4.3, ~ FA * Time)

# Apply custom contrasts
contrastsC4.1 = data.frame(contrast(emmC4.1, method = contrast_list, adjust = "none"))
contrastsC4.2 = data.frame(contrast(emmC4.2, method = contrast_list, adjust = "none"))
contrastsC4.3 = data.frame(contrast(emmC4.3, method = contrast_list, adjust = "none"))

contrastsC4.1$Outcome = v4.1
contrastsC4.2$Outcome = v4.2
contrastsC4.3$Outcome = v4.3

clusterC4res = rbind(contrastsC4.1, contrastsC4.2, contrastsC4.3)
clusterC4res = clusterC4res %>% arrange(p.value)
...
clusterC4res %>% filter(BH < 0.06) %>% filter(Outcome == v4.3)


```


Estimate and apply corrections for Cluster 5. Primarily significant differences within group over time.
```{r}

emmC5.1 = emmeans(AM_C5.1, ~ FA * Time)
emmC5.2 = emmeans(AM_C5.2, ~ FA * Time)
emmC5.3 = emmeans(AM_C5.3, ~ FA * Time)
emmC5.4 = emmeans(AM_C5.4, ~ FA * Time)
emmC5.5 = emmeans(AM_C5.5, ~ FA * Time)
emmC5.6 = emmeans(AM_C5.6, ~ FA * Time)

# Apply custom contrasts
contrastsC5.1 = data.frame(contrast(emmC5.1, method = contrast_list, adjust = "none"))
contrastsC5.2 = data.frame(contrast(emmC5.2, method = contrast_list, adjust = "none"))
contrastsC5.3 = data.frame(contrast(emmC5.3, method = contrast_list, adjust = "none"))
contrastsC5.4 = data.frame(contrast(emmC5.4, method = contrast_list, adjust = "none"))
contrastsC5.5 = data.frame(contrast(emmC5.5, method = contrast_list, adjust = "none"))
contrastsC5.6 = data.frame(contrast(emmC5.6, method = contrast_list, adjust = "none"))

contrastsC5.1$Outcome = v5.1
contrastsC5.2$Outcome = v5.2
contrastsC5.3$Outcome = v5.3
contrastsC5.4$Outcome = v5.4
contrastsC5.5$Outcome = v5.5
contrastsC5.6$Outcome = v5.6

clusterC5res = rbind(contrastsC5.1, contrastsC5.2, contrastsC5.3, contrastsC5.4, contrastsC5.5, contrastsC5.6)
clusterC5res = clusterC5res %>% arrange(p.value)
...
clusterC5res %>% filter(BH < 0.06) %>% filter(Outcome == v5.3)

clusterC5res %>% filter(BH < 0.06) %>% filter(Outcome == v5.4)

clusterC5res %>% filter(BH < 0.06) %>% filter(Outcome == v5.5)

clusterC5res %>% filter(BH < 0.06) %>% filter(Outcome == v5.6)
...
combinations = data.frame(emmeans(BM_C1.1, ~ FA * Time))
combination_names = paste(combinations$FA, combinations$Time, sep = "_")
...
```{r}
n1B
emmC1.1 = emmeans(BM_C1.1, ~ FA * Time)
emmC1.2 = emmeans(BM_C1.2, ~ FA * Time)
emmC1.3 = emmeans(BM_C1.3, ~ FA * Time)
...
contrastsC1.1$Outcome = v1.1B
contrastsC1.2$Outcome = v1.2B
contrastsC1.3$Outcome = v1.3B

clusterC1resB = rbind(contrastsC1.1, contrastsC1.2, contrastsC1.3)
clusterC1resB = clusterC1resB %>% arrange(p.value)

clusterC1resB$BH = p.adjust(clusterC1resB$p.value, method = "BH", n = length(clusterC1resB$p.value))
clusterC1resB$BY = p.adjust(clusterC1resB$p.value, method = "BY", n = length(clusterC1resB$p.value))

clusterC1resB$expestimate = exp(clusterC1resB$estimate)

clusterC1resB %>% filter(BH < 0.06) %>% filter(Outcome == v1.1B)

clusterC1resB %>% filter(BH < 0.06) %>% filter(Outcome == v1.2B)

clusterC1resB %>% filter(BH < 0.06) %>% filter(Outcome == v1.3B)
...
emmC2.1 = emmeans(BM_C2.1, ~ FA * Time)
emmC2.2 = emmeans(BM_C2.2, ~ FA * Time)

# Apply custom contrasts
contrastsC2.1 = data.frame(contrast(emmC2.1, method = contrast_list, adjust = "none"))
contrastsC2.2 = data.frame(contrast(emmC2.2, method = contrast_list, adjust = "none"))

contrastsC2.1$Outcome = v2.1B
contrastsC2.2$Outcome = v2.2B

clusterC2resB = rbind(contrastsC2.1, contrastsC2.2)
clusterC2resB = clusterC2resB %>% arrange(p.value)

clusterC2resB$BH = p.adjust(clusterC2resB$p.value, method = "BH", n = length(clusterC2resB$p.value))
clusterC2resB$BY = p.adjust(clusterC2resB$p.value, method = "BY", n = length(clusterC2resB$p.value))

clusterC2resB$expestimate = exp(clusterC2resB$estimate)

clusterC2resB %>% filter(BH < 0.06) %>% filter(Outcome == v2.1B)

clusterC2resB %>% filter(BH < 0.06) %>% filter(Outcome == v2.2B)
...
mat4.2A_mva = mat4.2A %>% filter(Group == "MVA85A")
mat4.2A_qft = mat4.2A %>% filter(Group == "QFT")
mat4.2A_fa = mat4.2A %>% filter(Group %in% c("FA - FA", "FA - BA"))

mat4.2B_mva = mat4.2B %>% filter(Group == "MVA85A")
mat4.2B_qft = mat4.2B %>% filter(Group == "QFT")
mat4.2B_fa = mat4.2B %>% filter(Group %in% c("FA - FA", "FA - BA"))

# MVA85A data preparation

mat4.2A_mva_wide = mat4.2A_mva %>%
  pivot_wider(names_from = Coefficient, id_cols = OutcomeGroup, values_from = value)

mat4.2A2_mva = as.matrix(mat4.2A_mva_wide[, 2:4], nrow = nrow(mat4.2A_mva_wide), ncol = 3)
row.names(mat4.2A2_mva) = mat4.2A_mva_wide$OutcomeGroup
dist4.2A_mva = get_dist(mat4.2A2_mva, "pearson")

mat4.2B_mva_wide = mat4.2B_mva %>%
  pivot_wider(names_from = Coefficient, id_cols = OutcomeGroup, values_from = value)

mat4.2B2_mva = as.matrix(mat4.2B_mva_wide[, 2:3], nrow = nrow(mat4.2B_mva_wide), ncol = 2)
row.names(mat4.2B2_mva) = mat4.2B_mva_wide$OutcomeGroup
dist4.2B_mva = get_dist(mat4.2B2_mva, "pearson")

# QFT data preparation

mat4.2A_qft_wide = mat4.2A_qft %>%
  pivot_wider(names_from = Coefficient, id_cols = OutcomeGroup, values_from = value)

mat4.2A2_qft = as.matrix(mat4.2A_qft_wide[, 2:4], nrow = nrow(mat4.2A_qft_wide), ncol = 3)
row.names(mat4.2A2_qft) = mat4.2A_qft_wide$OutcomeGroup
dist4.2A_qft = get_dist(mat4.2A2_qft, "pearson")

mat4.2B_qft_wide = mat4.2B_qft %>%
  pivot_wider(names_from = Coefficient, id_cols = OutcomeGroup, values_from = value)

mat4.2B2_qft = as.matrix(mat4.2B_qft_wide[, 2:3], nrow = nrow(mat4.2B_qft_wide), ncol = 2)
row.names(mat4.2B2_qft) = mat4.2B_qft_wide$OutcomeGroup
dist4.2B_qft = get_dist(mat4.2B2_qft, "pearson")

# FA data preparation

mat4.2A_fa_wide = mat4.2A_fa %>%
  pivot_wider(names_from = Coefficient, id_cols = OutcomeGroup, values_from = value)

mat4.2A2_fa = as.matrix(mat4.2A_fa_wide[, 2:4], nrow = nrow(mat4.2A_fa_wide), ncol = 3)
row.names(mat4.2A2_fa) = mat4.2A_fa_wide$OutcomeGroup
dist4.2A_fa = get_dist(mat4.2A2_fa, "pearson")

mat4.2B_fa_wide = mat4.2B_fa %>%
  pivot_wider(names_from = Coefficient, id_cols = OutcomeGroup, values_from = value)

mat4.2B2_fa = as.matrix(mat4.2B_fa_wide[, 2:3], nrow = nrow(mat4.2B_fa_wide), ncol = 2)
row.names(mat4.2B2_fa) = mat4.2B_fa_wide$OutcomeGroup
dist4.2B_fa = get_dist(mat4.2B2_fa, "pearson")
...
res4.2.hclust_mva = as.hclust(agnes(dist4.2A_mva, diss = TRUE, method = "average"))
res4.2.hclust_qft = as.hclust(agnes(dist4.2A_qft, diss = TRUE, method = "average"))
res4.2.hclust_fa = as.hclust(agnes(dist4.2A_fa, diss = TRUE, method = "average"))

res4.2.hclust_mvaB = as.hclust(agnes(dist4.2B_mva, diss = TRUE, method = "average"))
res4.2.hclust_qftB = as.hclust(agnes(dist4.2B_qft, diss = TRUE, method = "average"))
res4.2.hclust_faB = as.hclust(agnes(dist4.2B_fa, diss = TRUE, method = "average"))
...
hcdata4.2_mva = dendro_data_k(res4.2.hclust_mva, 5)

p = plot_ggdendro(hcdata4.2_mva,
                   direction   = "rl",
                   expand.y    = 0.25) 
p + ylab("Link Height") + geom_hline(yintercept = 0.2) + theme(axis.title.y = element_blank())

hcdata4.2_qft = dendro_data_k(res4.2.hclust_qft, 6)

p = plot_ggdendro(hcdata4.2_qft,
                   direction   = "rl",
                   expand.y    = 0.25) 
p + ylab("Link Height") + geom_hline(yintercept = 0.2) + theme(axis.title.y = element_blank())

hcdata4.2_fa = dendro_data_k(res4.2.hclust_fa, 7)

p = plot_ggdendro(hcdata4.2_fa,
                   direction   = "rl",
                   expand.y    = 0.25) 
p + ylab("Link Height") + geom_hline(yintercept = 0.2) + theme(axis.title.y = element_blank())
...
clusters4.2A = cutree(res4.2.hclust_mva, k=5)
...
mat4.2A_mva1 = mat4.2A_mva_wide %>% filter(OutcomeGroup %in% names1)

pca4.2A_mat = as.matrix(mat4.2A_mva1[, 2:4], nrow = nrow(mat4.2A_mva1), ncol = 3)
rownames(pca4.2A_mat) = mat4.2A_mva1$OutcomeGroup
...
PC1 primarily describes differences after BCG.
...
fviz_contrib(pca4.2Ares, axes = 1, choice = "ind", fill = "darkgrey", color = "black", top = 20) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
  theme(title = element_blank(),
        axis.title.y = element_text(size = 12, face = "bold"),
...
One PC is sufficient
```{r}

mat4.2A_mva2 = mat4.2A_mva_wide %>% filter(OutcomeGroup %in% names2)

pca4.2A_mat = as.matrix(mat4.2A_mva2[, 2:4], nrow = nrow(mat4.2A_mva2), ncol = 3)
rownames(pca4.2A_mat) = mat4.2A_mva2$OutcomeGroup
...
```
Primarily differences prior to BCG

```{r}

fviz_contrib(pca4.2Ares, axes = 1, choice = "var", fill = "darkgrey", color = "black", top = 50) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
  theme(title = element_blank(),
        axis.title.y = element_text(size = 12, face = "bold"),
...
Two PCs are sufficient

```{r}

mat4.2A_mva3 = mat4.2A_mva_wide %>% filter(OutcomeGroup %in% names3)

pca4.2A_mat = as.matrix(mat4.2A_mva3[, 2:4], nrow = nrow(mat4.2A_mva3), ncol = 3)
rownames(pca4.2A_mat) = mat4.2A_mva3$OutcomeGroup
...
```
PCs separate effect 8 weeks after BCG and effect before and 1 year later.
```{r}
fviz_contrib(pca4.2Ares, axes = 1, choice = "var", fill = "darkgrey", color = "black", top = 50) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
...
fviz_contrib(pca4.2Ares, axes = 2, choice = "var", fill = "darkgrey", color = "black", top = 50) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
  theme(title = element_blank(),
        axis.title.y = element_text(size = 12, face = "bold"),
...
fviz_contrib(pca4.2Ares, axes = 2, choice = "ind", fill = "darkgrey", color = "black", top = 50) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
  theme(title = element_blank(),
        axis.title.y = element_text(size = 12, face = "bold"),
...
mat4.2A_mva4 = mat4.2A_mva_wide %>% filter(OutcomeGroup %in% names4)

pca4.2A_mat = as.matrix(mat4.2A_mva4[, 2:4], nrow = nrow(mat4.2A_mva4), ncol = 3)
rownames(pca4.2A_mat) = mat4.2A_mva4$OutcomeGroup
...
mat4.2A_mva5 = mat4.2A_mva_wide %>% filter(OutcomeGroup %in% names5)

pca4.2A_mat = as.matrix(mat4.2A_mva5[, 2:4], nrow = nrow(mat4.2A_mva5), ncol = 3)
rownames(pca4.2A_mat) = mat4.2A_mva5$OutcomeGroup
...
pc1dat = contrib4.2A.1$data[which(contrib4.2A.1$data$contrib >= 10), ]
pc2dat = contrib4.2A.2$data[which(contrib4.2A.2$data$contrib >= 10), ]

pcnames1 = as.character(unique(c(pc1dat$name, pc1dat$name)))
...
```{r}
sexclusters_mva = c(sex_mva_c1, sex_mva_c2, sex_mva_c3, sex_mva_c4, sex_mva_c5)
```

```{r}

clusters4.2A = cutree(res4.2.hclust_qft, k=6)
...
names6 = names(clusters4.2A[clusters4.2A == 1])
#names1split = unique(lapply(names1, drop_after_match, matches))

names4 = names(clusters4.2A[clusters4.2A == 2])
#names2split = unique(lapply(names2, drop_after_match, matches))

names3 = names(clusters4.2A[clusters4.2A == 3])
#names3split = unique(lapply(names3, drop_after_match, matches))

names5 = names(clusters4.2A[clusters4.2A == 4])
#names4split = unique(lapply(names4, drop_after_match, matches))

names2 = names(clusters4.2A[clusters4.2A == 5])
#names5split = unique(lapply(names5, drop_after_match, matches))

names1 = names(clusters4.2A[clusters4.2A == 6])


```

Set aside small clusters
...
mat4.2A_qft2 = mat4.2A_qft_wide %>% filter(OutcomeGroup %in% names4)

pca4.2A_mat = as.matrix(mat4.2A_qft2[, 2:4], nrow = nrow(mat4.2A_qft2), ncol = 3)
rownames(pca4.2A_mat) = mat4.2A_qft2$OutcomeGroup
...
```
PC1 describes differences before BCG and at 1 year of age. There is a single outcome. PC2 describes differences 8 weeks after BCG. There are 2 outcomes.
```{r}
fviz_contrib(pca4.2Ares, axes = 1, choice = "var", fill = "darkgrey", color = "black", top = 50) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
...
mat4.2A_qft5 = mat4.2A_qft_wide %>% filter(OutcomeGroup %in% names5)

pca4.2A_mat = as.matrix(mat4.2A_qft5[, 2:4], nrow = nrow(mat4.2A_qft5), ncol = 3)
rownames(pca4.2A_mat) = mat4.2A_qft5$OutcomeGroup
...
```
PC1 describes differences before BCG and at 1 year of age. There are 2 outcomes. PC2 describes differences 8 weeks after BCG. There are 2 outcomes.
```{r}
fviz_contrib(pca4.2Ares, axes = 1, choice = "var", fill = "darkgrey", color = "black", top = 50) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
...
mat4.2A_qft6 = mat4.2A_qft_wide %>% filter(OutcomeGroup %in% names6)

pca4.2A_mat = as.matrix(mat4.2A_qft6[, 2:4], nrow = nrow(mat4.2A_qft6), ncol = 3)
rownames(pca4.2A_mat) = mat4.2A_qft6$OutcomeGroup
...
```
PC1 describes differences at 1 year of age. There is a single outcome.
```{r}
fviz_contrib(pca4.2Ares, axes = 1, choice = "var", fill = "darkgrey", color = "black", top = 50) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
...
pc1dat = contrib4.2A.1$data[which(contrib4.2A.1$data$contrib >= 15), ]
...
clusters4.2A = cutree(res4.2.hclust_fa, k=7)
...
names7 = names(clusters4.2A[clusters4.2A == 1])
#names1split = unique(lapply(names1, drop_after_match, matches))

names4 = names(clusters4.2A[clusters4.2A == 2])
#names2split = unique(lapply(names2, drop_after_match, matches))

names2 = names(clusters4.2A[clusters4.2A == 3])
#names3split = unique(lapply(names3, drop_after_match, matches))

names6 = names(clusters4.2A[clusters4.2A == 4])
#names4split = unique(lapply(names4, drop_after_match, matches))

names1 = names(clusters4.2A[clusters4.2A == 5])
#names5split = unique(lapply(names5, drop_after_match, matches))

names3 = names(clusters4.2A[clusters4.2A == 6])

names5 = names(clusters4.2A[clusters4.2A == 7])
...
mat4.2A_fa1 = mat4.2A_fa_wide %>% filter(OutcomeGroup %in% names1)

pca4.2A_mat = as.matrix(mat4.2A_fa1[, 2:4], nrow = nrow(mat4.2A_fa1), ncol = 3)
rownames(pca4.2A_mat) = mat4.2A_fa1$OutcomeGroup
...
```
One immune outcome in one direction. Select the whole set.
```{r}
fviz_contrib(pca4.2Ares, axes = 1, choice = "var", fill = "darkgrey", color = "black", top = 50) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
...
mat4.2A_fa2 = mat4.2A_fa_wide %>% filter(OutcomeGroup %in% names2)

pca4.2A_mat = as.matrix(mat4.2A_fa2[, 2:4], nrow = nrow(mat4.2A_fa2), ncol = 3)
rownames(pca4.2A_mat) = mat4.2A_fa2$OutcomeGroup
...
```
PC1 describes differences after BCG. Two immune outcomes, both directions
```{r}
fviz_contrib(pca4.2Ares, axes = 1, choice = "var", fill = "darkgrey", color = "black", top = 50) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
...
mat4.2A_fa3 = mat4.2A_fa_wide %>% filter(OutcomeGroup %in% names3)

pca4.2A_mat = as.matrix(mat4.2A_fa3[, 2:4], nrow = nrow(mat4.2A_fa3), ncol = 3)
rownames(pca4.2A_mat) = mat4.2A_fa3$OutcomeGroup
...
mat4.2A_fa4 = mat4.2A_fa_wide %>% filter(OutcomeGroup %in% names4)

pca4.2A_mat = as.matrix(mat4.2A_fa4[, 2:4], nrow = nrow(mat4.2A_fa4), ncol = 3)
rownames(pca4.2A_mat) = mat4.2A_fa4$OutcomeGroup
...
```
PC1 describes differences at 1 year.
```{r}
fviz_contrib(pca4.2Ares, axes = 1, choice = "var", fill = "darkgrey", color = "black", top = 50) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
...
```{r}
contrib4.2A.1 = fviz_contrib(pca4.2Ares, axes = 1, choice = "ind")
contrib4.2A.1 = fviz_contrib(pca4.2Ares, axes = 2, choice = "ind")

pc1dat = contrib4.2A.1$data[which(contrib4.2A.1$data$contrib >= 5), ]
pc2dat = contrib4.2A.2$data[which(contrib4.2A.2$data$contrib >= 5), ]
...
mat4.2A_fa5 = mat4.2A_fa_wide %>% filter(OutcomeGroup %in% names5)

pca4.2A_mat = as.matrix(mat4.2A_fa5[, 2:4], nrow = nrow(mat4.2A_fa5), ncol = 3)
rownames(pca4.2A_mat) = mat4.2A_fa5$OutcomeGroup
...
One PCs is sufficient
```{r}

mat4.2A_fa6 = mat4.2A_fa_wide %>% filter(OutcomeGroup %in% names6)

pca4.2A_mat = as.matrix(mat4.2A_fa6[, 2:4], nrow = nrow(mat4.2A_fa6), ncol = 3)
rownames(pca4.2A_mat) = mat4.2A_fa6$OutcomeGroup
...
```
PC1 describes differences after BCG in one outcome.
```{r}
fviz_contrib(pca4.2Ares, axes = 1, choice = "var", fill = "darkgrey", color = "black", top = 50) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
...
mat4.2A_fa7 = mat4.2A_fa_wide %>% filter(OutcomeGroup %in% names7)

pca4.2A_mat = as.matrix(mat4.2A_fa7[, 2:4], nrow = nrow(mat4.2A_fa7), ncol = 3)
rownames(pca4.2A_mat) = mat4.2A_fa7$OutcomeGroup
...
Aset_list = list(MVA85A = mvaclusters, QFT = qftclusters, FA = faclusters)
...
hcdata4.2B_mva = dendro_data_k(res4.2.hclust_mvaB, 2)

p = plot_ggdendro(hcdata4.2B_mva,
                   direction   = "rl",
                   expand.y    = 0.25) 
p + ylab("Link Height") + geom_hline(yintercept = 0.2) + theme(axis.title.y = element_blank())

hcdata4.2B_qft = dendro_data_k(res4.2.hclust_qftB, 2)

p = plot_ggdendro(hcdata4.2B_qft,
                   direction   = "rl",
                   expand.y    = 0.25) 
p + ylab("Link Height") + geom_hline(yintercept = 0.2) + theme(axis.title.y = element_blank())

hcdata4.2B_fa = dendro_data_k(res4.2.hclust_faB, 2)

p = plot_ggdendro(hcdata4.2B_fa,
                   direction   = "rl",
                   expand.y    = 0.25) 
p + ylab("Link Height") + geom_hline(yintercept = 0.2) + theme(axis.title.y = element_blank())
...
clusters4.2B_mva = cutree(res4.2.hclust_mvaB, k=2)
clusters4.2B_qft = cutree(res4.2.hclust_qftB, k=2)
clusters4.2B_fa = cutree(res4.2.hclust_faB, k=2)

names2_mva = names(clusters4.2B_mva[clusters4.2B_mva == 1])
names1_mva = names(clusters4.2B_mva[clusters4.2B_mva == 2])

names2_qft = names(clusters4.2B_qft[clusters4.2B_qft == 1])
names1_qft = names(clusters4.2B_qft[clusters4.2B_qft == 2])

names2_fa = names(clusters4.2B_fa[clusters4.2B_fa == 1])
names1_fa = names(clusters4.2B_fa[clusters4.2B_fa == 2])
...
mat4.2B_mva1 = mat4.2B_mva_wide %>% filter(OutcomeGroup %in% names1_mva)

pca4.2B_mat = as.matrix(mat4.2B_mva1[, 2:3], nrow = nrow(mat4.2B_mva1), ncol = 2)
rownames(pca4.2B_mat) = mat4.2B_mva1$OutcomeGroup

pca4.2Bres = princomp(pca4.2B_mat, cor = TRUE, scores = TRUE)
...
```
Select 7 immune outcomes

```{r}

fviz_contrib(pca4.2Bres, axes = 1, choice = "var", fill = "darkgrey", color = "black", top = 50) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
  theme(title = element_blank(),
        axis.title.y = element_text(size = 12, face = "bold"),
...
fviz_contrib(pca4.2Bres, axes = 2, choice = "var", fill = "darkgrey", color = "black", top = 50) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
  theme(title = element_blank(),
        axis.title.y = element_text(size = 12, face = "bold"),
...
fviz_contrib(pca4.2Bres, axes = 2, choice = "ind", fill = "darkgrey", color = "black", top = 50) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
  theme(title = element_blank(),
        axis.title.y = element_text(size = 12, face = "bold"),
...
```{r}
contrib4.2B.1 = fviz_contrib(pca4.2Bres, axes = 1, choice = "ind")
contrib4.2B.2 = fviz_contrib(pca4.2Bres, axes = 2, choice = "ind")

pc1dat = contrib4.2B.1$data[which(contrib4.2B.1$data$contrib >= 10), ]
pc2dat = contrib4.2B.2$data[which(contrib4.2B.2$data$contrib >= 10), ]

pcnames1 = as.character(unique(c(pc1dat$name, pc2dat$name)))

sex_mvaB_c1 = pcnames1
...
mat4.2B_mva2 = mat4.2B_mva_wide %>% filter(OutcomeGroup %in% names2_mva)

pca4.2B_mat = as.matrix(mat4.2B_mva2[, 2:3], nrow = nrow(mat4.2B_mva2), ncol = 2)
rownames(pca4.2B_mat) = mat4.2B_mva2$OutcomeGroup
...
Select 2 immune outcomes and drop the third
...
pc1dat = contrib4.2B.1$data[which(contrib4.2B.1$data$contrib >= 20), ]
pc2dat = contrib4.2B.2$data[which(contrib4.2B.2$data$contrib >= 20), ]

pcnames1 = as.character(unique(c(pc1dat$name, pc2dat$name)))

sex_mvaB_c2 = pcnames1

```

```{r}
sexclusters_mvaB = c(sex_mvaB_c1, sex_mvaB_c2)
```
...
```{r}
mat4.2B_qft1 = mat4.2B_qft_wide %>% filter(OutcomeGroup %in% names1_qft)

pca4.2B_mat = as.matrix(mat4.2B_qft1[, 2:3], nrow = nrow(mat4.2B_qft1), ncol = 2)
rownames(pca4.2B_mat) = mat4.2B_qft1$OutcomeGroup
...
Select five immune outcomes -  1 contributes to both PCs
```{r}
fviz_contrib(pca4.2Bres, axes = 1, choice = "var", fill = "darkgrey", color = "black", top = 50) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
...
sex_qftB_c1 = pcnames1

```

```{r}
mat4.2B_qft2 = mat4.2B_qft_wide %>% filter(OutcomeGroup %in% names2_qft)

pca4.2B_mat = as.matrix(mat4.2B_qft2[, 2:3], nrow = nrow(mat4.2B_qft2), ncol = 2)
rownames(pca4.2B_mat) = mat4.2B_qft2$OutcomeGroup
...
```{r}
sexclusters_qftB = c(sex_qft_c1, names2_qft)
```
...
```{r}
mat4.2B_fa1 = mat4.2B_fa_wide %>% filter(OutcomeGroup %in% names1_fa)

pca4.2B_mat = as.matrix(mat4.2B_fa1[, 2:3], nrow = nrow(mat4.2B_fa1), ncol = 2)
rownames(pca4.2B_mat) = mat4.2B_fa1$OutcomeGroup
...
Select two immune outcomes from PC1
```{r}
fviz_contrib(pca4.2Bres, axes = 1, choice = "var", fill = "darkgrey", color = "black", top = 50) +
  labs(y = element_text("Contributions(%)\n")) +
  theme_minimal() +
...
pc1dat = contrib4.2B.1$data[which(contrib4.2B.1$data$contrib >= 8), ]
#pc2dat = contrib4.2B.2$data[which(contrib4.2B.2$data$contrib >= 10), ]
...
One PC is sufficient
```{r}
mat4.2B_fa2 = mat4.2B_fa_wide %>% filter(OutcomeGroup %in% names2_fa)

pca4.2B_mat = as.matrix(mat4.2B_fa2[, 2:3], nrow = nrow(mat4.2B_fa2), ncol = 2)
rownames(pca4.2B_mat) = mat4.2B_fa2$OutcomeGroup
...
```{r}
sexclusters_faB = c(sex_faB_c1, sex_faB_c2)
```
